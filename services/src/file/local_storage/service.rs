    ...
    async fn run(&self, args: Self::Args) -> Result<Self::Out, FileServiceError> {
        ...
        match args {
            FileAction::Save { bytes, dst } => {
                path_file.push(&dst);
                let mut new_file =
                    File::create_new(path_file).map_err(|e| FileServiceError(e.to_string()))?;

                new_file
                    .write_all(&bytes)
                    .map_err(|e| FileServiceError(e.to_string()))?;
            ...
            FileAction::Open { path } => {
                path_file.push(&path);
                let file = File::open(path_file).map_err(|e| FileServiceError(e.to_string()))?;
            ...
            FileAction::Move { src, dst, copy } => {
                ...
                path_dst.push(&dst);
                {
                    ...
                    let mut file = File::open(src).map_err(|e| FileServiceError(e.to_string()))?;
                    let _ = file.read_to_end(&mut bytes);

                    let mut new_file =
                        File::create_new(dst).map_err(|e| FileServiceError(e.to_string()))?;
            ...
            FileAction::Delete { src } => {
                path_file.push(&src);
                fs::remove_file(path_file).map_err(|e| FileServiceError(e.to_string()))?;
